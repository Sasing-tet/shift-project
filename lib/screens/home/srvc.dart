...
class Srvc{    
  static Future<void> drawRoadManually(List<FloodMarkerRoute> routesOnPolylines, MapController mapController) async {
...
static Future<void> removeAllMarkers(List<FloodMarkerRoute> routesOnPolylines, MapController mapController) async {
  for (var i = 0; i < routesOnPolylines.length; i++) {
    final routeOnPolyline = routesOnPolylines[i];
    final riskPoints = routeOnPolyline.points;
...
static Future<List<List<GeoPoint>>> fetchOSRMRoutePolylines(
  ...
  static Future<List<FloodMarkerRoute>> getRoutesOnPolylines(
    ...
    MapController mapController, // Pass the mapController for the toListGeo() function
    {double epsilon = 1e-8}
  ) async {
        ...
        for (var groupPoints in pointGroups) {
          ...
          for (var point in groupPoints) {
            for (int i = 0; i < polyline.length - 1; i++) {
              if (isBetween(polyline[i], polyline[i + 1], point, epsilon: epsilon)) {
...
static Future<void> addMarkersToMap(List<FloodMarkerPoint>? pointsOnPolyline, MapController mapController) async {
...
static Future<void> removeMarker(List<FloodMarkerPoint>? pointsOnPolyline, MapController mapController) async {
  if (pointsOnPolyline == null) {
    return;
  }
...
static List<List<GeoPoint>> extractGeoPoints(String geoJsonString) {
  ...
  static List<FloodMarkerPoint> processDataAndAddToMarkerPoints() {
    List<List<GeoPoint>> highGeoPoints = Srvc.extractGeoPoints(highfloodgeojson);
...
static Future<bool> isWithinFloodProneArea(GeoPoint point, double maxDistance, List<GeoPoint> polyline) async {
  for (int i = 0; i < polyline.length - 1; i++) {
...
static Future<double> calculateDistanceToSegment(GeoPoint point, GeoPoint segmentStart, GeoPoint segmentEnd) async {
...
static Future<void> checkFloodProneArea(GeoPoint userLocation, double maxDistance, List<FloodMarkerPoint>? floodProneArea, context) async {
  ...
  static Future<void> updateLocation(FloodMarkerRoute routeCHOSEN, MapController mapController,AnimationController _animationController, context) async {
    if (!routeCHOSEN.route.isEmpty) {
     
      final myposition = await mapController.myLocation();

      final dist =
          await distance2point(myposition, routeCHOSEN.route.last);
      ...
      Future.delayed(Duration(seconds: 1), () => updateLocation(routeCHOSEN, mapController,_animationController,context));
    }
  }
