...
class Ops {
  static Future<void> drawRoadManually(
      List<String> encodedPolylines, MapController mapController) async {
  ...
  static Future<Map<String, List<List<GeoPoint>>>> getPointsOnPolylines(
      List<String> polylines, Map<String, List<List<GeoPoint>>> markerPoints,
        ...
        for (var groupPoints in pointGroups) {
          ...
          for (var markerPoint in groupPoints) {
            for (int i = 0; i < polyline.length - 1; i++) {
              if (isBetween(polyline[i], polyline[i + 1], markerPoint,
                  epsilon: epsilon)) {
  ...
  static void addMarkersToMap(
      Map<String, List<List<GeoPoint>>> pointsOnPolyline,
...
//             debugPrint("${riskpointGroups.length}");
//             debugPrint("${riskpointGroups.toList()}");
...
//             }
//           }
//           if (riskpointGroups.isNotEmpty) {debugPrint("level $level");
//             debugPrint("${riskpointGroups.length}");
//             debugPrint("${riskpointGroups.toList()}");
//             pointsOnPolyline.add(FloodMarkerPoint(level, riskpointGroups));
//           }
